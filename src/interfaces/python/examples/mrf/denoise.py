...
def denoiseModel(
    ...
    **Denoising and inpainting problems:** from `Mrf- Benchmark <http://vision.middlebury.edu/MRF/results/ >`_
    ...
    numberOfPairwiseFactors = shape[0] * \
        (shape[1] - 1) + shape[1] * (shape[0] - 1)
    ...
    pixelValueRep = numpy.repeat(allPossiblePixelValues[
                                 :, numpy.newaxis], numLabels, 1)
    ...
    f = opengm.differenceFunction(
        shape=[numLabels, numLabels], norm=2, weight=weight, truncate=truncate)
...
if __name__ == "__main__":
    ...
    # Read image
    img = numpy.array(numpy.squeeze(vigra.impex.readImage(
        imgPath)), dtype=opengm.value_type)  # [0:100,0:40]
    shape = img.shape
