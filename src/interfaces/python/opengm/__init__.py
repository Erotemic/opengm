...
_solverDicts = [
    ...
    (inference.multiplier.integrator.solver.__dict__, 'adder',
        'integrator'),  # todo(jyunes) XXX should this be adder instead?
...
class __RandomFusion__(object):
    ...
    def infer(self, visitor=None):
        if(self.arg_ is None):
            self.arg_ = numpy.zeros(
                self.gm_.numberOfVariables, dtype=label_type)
        ...
        for x in range(self.steps):
            randState = numpy.random.randint(
                low=0, high=self.nLabels, size=self.nVar).astype(label_type)
