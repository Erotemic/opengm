    ...
    try:
        ...
        if 'names' in classDict:
            ...
            if(isinstance(names, dict)):
                # TODO: replace type check : check if type is the same as for
                # an actual boost python enum
...
def to_native_boost_python_enum_converter(givenValue, nativeClass):
    if is_boost_python_enum(instanceType=givenValue) or is_string(instanceType=givenValue):
        ...
        raise TypeError('Cannot find the given name \'%s\' in the native enums names %s' % (
            str(givenValue), str(nativeClass.names.keys())))
        ...
        if asInt in nativeClass.values:
            ...
            raise TypeError('Cannot find the given value \'%i\' in the native enums values %s' % (
                asInt, str(nativeClass.values)))

    elif same_class(classA=nativeClass, instanceB=givenValue):
        return givenValue
    else:
        raise TypeError('Cannot convert given value \'%s\' of the type \'%s\' to an boost python enum' % (
            str(givenValue), str(givenValue.__class__)))
...
def to_native_build_in_simple_class_converter(givenValue, nativeClass):
    ...
    if is_build_in_simple_parameter(instanceType=givenValue):
        ...
        raise TypeError('Cannot convert the class \'%s\' to the native class \'%s\'' % (
            str(givenValue.__class__), str(nativeClass)))
...
def to_native_tribool_converter(givenValue, nativeClass):
    if isinstance(givenValue, (bool, int)):
        ...
        raise TypeError('Cannot find the given name \'%s\' into  %s' % (
            str(givenValue), str(['True', 'False', 'Maybe'])))
        ...
        if is_meta_inf_param(instanceType=givenValue):
            ...
            for name in givenKwargs:
                if name in allowedKwargs:
                    ...
                    raise TypeError("Inference Parameter %s has no attribute %s" % (
                        str(nativeClass), name))
        else:
            raise TypeError("Inference Parameter %s cannot be constructed from  %s" % (
                str(givenValue.__class__), name))
        return cppParameter
...
def to_native_class_converter(givenValue, nativeClass, converterPolicy=None):
    ...
    # givenValue == list
        ...
        raise RuntimeError(
            "for now: nativeClass and givenValue must be the same")
