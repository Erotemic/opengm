class Inference:
    ...
    def __init__(self, gm, alg, impl=None, parameter=None, accumulator=None, constructSolver=True):
        ...
        possibleInferenceFamilyClasses = _CppInferenceAlgorithms.inferenceDict[
            self.gm.operator][self.accumulator]
        if alg in possibleInferenceFamilyClasses:
            inferenceFamilyClasses = possibleInferenceFamilyClasses[alg]
        else:
            supportedAlgorithmsStr = _CppInferenceAlgorithms.supportedAlgorithmsStr(
                gm.operator, accumulator)
        ...
        if parameter is not None:
            self.cppParameter = to_native_class_converter(
                givenValue=parameter, nativeClass=self.cppParameterClass)
            """
         # set up parameter
