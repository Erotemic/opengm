        ...
        class _more_gm(_InjectorGm, gmClass):
            ...
            def factorSubset(self, factorIndices=None, order=None):
                ...
                if factorIndices is None:
                    fIndices = numpy.arange(
                        self.numberOfFactors, dtype=index_type)
                else:
                    fIndices = numpy.require(factorIndices, dtype=index_type)
                if order is None:
                    return FactorSubset(self, fIndices)
                else:
                    factorIndicesWithOrder = self._factor_withOrder(
                        fIndices, int(order))
            ...
            def evaluate(self, labels):
                ...
                if len(labels) != self.numberOfVariables:
                    ...
                    nGiven = len(labels)
                    raise RuntimeError(
                        'number of given labels (%d) does not match gm.numberOfVariables (%d)' % (nGiven, nVar))
                if isinstance(labels, numpy.ndarray):
                    ...
                    raise RuntimeError(
                        "%s is not an supperted type for arument ``labels`` in ``evaluate``" % (str(type(labels)),))
            ...
            def addFactors(self, fids, variableIndices, finalize=True):
                ...
                if (isinstance(variableIndices, numpy.ndarray)):
                    ...
                    try:
                        ...
                        raise RuntimeError("%s is not an supperted type for arument ``variableIndices`` in ``addFactors``" % (
                            str(type(variableIndices)),))
                ...
                if isinstance(functions, numpy.ndarray):
                    ...
                    try:
                        ...
                        try:
                            ...
                            raise RuntimeError(
                                "%s is an not a supported type for addFunctions " % (str(type(functions)),))
