                ...
                for b in bases:
                    ...
                    if(issubclass(b, solverParamClass)):
                        ...
                        def newInit(*args, **kwargs):
                            ...
                            baseInit(self_proxy)
                            self_proxy._after_init(
                                *args[1:len(args)], **kwargs)

                        def newSetAttr(*args, **kwargs):
                            self_proxy = args[0]
                            assert isinstance(self_proxy, solverParamClass)
                            name, nativeValue = self_proxy._setattr_helper__(
                                *args[1:len(args)], **kwargs)
                            print(nativeValue)
                            super(solverParamClass, self_proxy).__setattr__(
                                name, nativeValue)
    ...
    class PyAddon_GenericInferenceParameter(InjectorGenericInferenceParameter, solverParamClass):
        ...
        def _setattr_helper__(self, name, value):
            if hasattr(self, name):
                ...
                raise NameError("%s has no attribute named \'%s\' , the parameter attributes are : %s " % (
                    str(solverParamClass), str(name), str(self.getParameterNames())))
                ...
                if((propertyName.startswith('__') or propertyName.endswith('__')) == False):
                    ...
                    if(repr(value).find('property') != -1):
                        ...
                        if(propertyName == 'subInfParam'):
                            sys.stdout.write(str("\n\n\n"))
                            sys.stdout.write(
                                str(attrValue._str_spaced_(space + '   ')))
                        else:
                            sys.stdout.write(str(attrValue))
                            sys.stdout.write("\n\n\n")
                            sys.stdout.write(space + '   ')

                            docStrAttr = str(solverParamClass.__dict__[
                                             propertyName].__doc__)
