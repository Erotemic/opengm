...
def visualizeGm(gm, plotUnaries=True, plotFunctions=False, plotNonShared=False, layout='neato', iterations=1000, show=True, relNodeSize=1.0):
    ...
    # [x for x in xrange(gm.numberOfVariables,gm.numberOfVariables+gm.numberOfFactors)]
    factorNodeList = []
    functionNodeList = []
    # edge list
    factorVarEdgeList = []
    ...
    if plotFunctions:
        nx.draw_networkx(G, pos, node_size=nodeSize * 2.0, nodelist=functionNodeList, fontSize=fontSize, withLabels=False,
                         labels=functionLabels, node_color='gray', font_color='k', edgelist=[], node_shape='d', font_size=fontSize * 0.75)
        nx.draw_networkx_edges(G, pos, alpha=0.9, width=1 * relNodeSize,
                               edgelist=factorFunctionEdgeList, style='dotted')

    nx.draw_networkx(G, pos, node_size=nodeSize, nodelist=factorNodeList, fontSize=fontSize, withLabels=False,
                     labels=factorLabels, node_color='k', font_color='w', edgelist=[], node_shape='s', font_size=fontSize)
    nx.draw_networkx(G, pos, node_size=nodeSize, nodelist=varNodeList, fontSize=fontSize, withLabels=False,
                     labels=varLabels, node_color='w', font_color='k', edgelist=[], node_shape='o', font_size=fontSize)

    nx.draw_networkx_edges(G, pos, alpha=0.6, width=1 *
                           relNodeSize, edgelist=factorVarEdgeList, style='solid')
    plt.subplots_adjust()
    plt.axis('off')
