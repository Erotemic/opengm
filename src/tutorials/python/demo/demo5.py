...
if __name__ == "__main__":
    ...
    if len(args) != 11:
        print("Usage: ",  args[
              0], " infile outfile red1 green1 blue1 red2 green2 blue2 T lambda")
        sys.exit(0)
    ...
    protoColor1 = numpy.array(
        [args[3], args[4], args[5]], dtype=opengm.value_type).reshape([3, -1])
    protoColor1 = numpy.repeat(protoColor1, numVar, axis=1).swapaxes(0, 1)

    protoColor2 = numpy.array(
        [args[6], args[7], args[8]], dtype=opengm.value_type).reshape([3, -1])
    ...
    if False:
        ...
        inf = opengm.inference.BeliefPropagation(
            gm, parameter=opengm.InfParam(damping=0.9))
    ...
    class PyCallback(object):
        ...
        def __init__(self, shape, numLabels):
            ...
            self.cmap = matplotlib.colors.ListedColormap(
                numpy.random.rand(self.numLabels, 3))
        ...
        def visit(self, inference):
            ...
            plt.imshow(labelVector * 255.0, cmap=self.cmap,
